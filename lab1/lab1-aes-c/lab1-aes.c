...
//请补充代码
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            array[j][i] = getNumFromSBox(array[j][i]);
        }
    }
}
...
//请补充代码
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            array[j][i] = getNumFromS1Box(array[j][i]);
...
//请补充代码
    ...
    for(int i=0; i<4; i++){
        for(int j=0; j<4; j++){
            for(int k=0; k<4; k++){
                tmp = tmp ^ GFMul(deColM[i][k], a[k][j]);
            }
            array[i][j] = tmp;
            tmp = 0;
        }
...
void addRoundKey(int array[4][4], int round)
{
  ...
  //请补充代码
    ...
    int base = round*4;
    for(int i=0; i<4; i++){
        splitIntToArray(w[base + i], key_array);
        for(int j=0; j<4; j++){
            // key[j][i] = key_array[j];
            array[j][i] = array[j][i] ^ key_array[j];
        }
    }
    // for(int i=0; i<4; i++){
    //     for(int j=0; j<4; j++){
    //         array[j][i] = array[j][i] ^ key[j][i];
    //     }
    // }
}
...
//请补充代码
    ...
    //字节替换
    for(int i=0; i<4; i++){
        key_array[i] = getNumFromSBox(key_array[i]);
    }
    //异或
    result = mergeArrayToInt(key_array) ^ Rcon[round-1];
    return result;
